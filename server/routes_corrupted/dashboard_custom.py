...
dashboard_custom_bp = Blueprint('dashboard_custom', __name__)
...
@dashboard_custom_bp.route('/api/dashboard/config', methods=['GET'])
def get_dashboard_config():
    try:
        ...
        # DUPLICATE REMOVED: @dashboard_custom_bp.route('/api/dashboard/config', methods=['GET'])
        # DUPLICATE REMOVED: def update_dashboard_config():
        ...
        @dashboard_custom_bp.route('/api/dashboard/tree/<category>', methods=['GET'])
...
def get_tree_hierarchy(category):
    try:
        ...
        @dashboard_custom_bp.route('/api/dashboard/tree/<category>/nodes', methods=['POST'])
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def add_tree_node(category):
    try:
        ...
        @dashboard_custom_bp.route('/api/dashboard/tree/<category>/nodes/<node_id>', methods=['PUT'])
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def update_tree_node(category, node_id):
    try:
        ...
        # DUPLICATE REMOVED: @dashboard_custom_bp.route('/api/dashboard/tree/<category>/nodes/<node_id>', methods=['GET'])
        # DUPLICATE REMOVED: def delete_tree_node(category, node_id):
        # DUPLICATE REMOVED:     """Delete a tree node"""
        # DUPLICATE REMOVED:     config = _load_dashboard_config()
        # DUPLICATE REMOVED:     
        # DUPLICATE REMOVED:     if category not in config['tree_hierarchy']:
        # DUPLICATE REMOVED:         return jsonify({'ok': False, 'error': 'Category not found'}), 404
        # DUPLICATE REMOVED:     
        # DUPLICATE REMOVED:     nodes = config['tree_hierarchy'][category]['nodes']
        # DUPLICATE REMOVED:     original_count = len(nodes)
        # DUPLICATE REMOVED:     
        # Remove node and its children
        # DUPLICATE REMOVED:     nodes_to_remove = [node_id]
        # DUPLICATE REMOVED:     
        # Find all children recursively
        # DUPLICATE REMOVED:     def find_children(parent_id):
        ...
        @dashboard_custom_bp.route('/api/dashboard/pinned-cards', methods=['GET'])
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def get_pinned_cards():
    try:
        ...
        @dashboard_custom_bp.route('/api/dashboard/pinned-cards/<card_id>', methods=['PUT'])
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def update_pinned_card(card_id):
    try:
        ...
        @dashboard_custom_bp.route('/api/dashboard/pinned-cards/<card_id>/toggle', methods=['POST'])
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def toggle_pinned_card(card_id):
    try:
        ...
        @dashboard_custom_bp.route('/api/dashboard/pinned-cards/reorder', methods=['POST'])
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

def reorder_pinned_cards():
    try:
        ...
        order = req_data['order']  # Array of card IDs in desired order
        ...
        @dashboard_custom_bp.route('/api/dashboard/layout', methods=['GET'])
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def get_layout_preferences():
    try:
        ...
        # DUPLICATE REMOVED: @dashboard_custom_bp.route('/api/dashboard/layout', methods=['POST'])
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def create_dashboard_layout():
    ...
    try:
        ...
        return jsonify({'error': str(e)}), 500
# DUPLICATE REMOVED: def update_layout_preferences():
