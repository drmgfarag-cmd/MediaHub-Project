...
@subtitles_advanced_bp.route('/api/subtitles/config', methods=['POST'])
def set_config():
    """
...
@subtitles_advanced_bp.route('/api/subtitles/priority/global', methods=['POST'])
def set_global_priority():
    """
...
@subtitles_advanced_bp.route('/api/subtitles/priority/category/<category>', methods=['POST'])
def set_category_priority(category):
    """
...
@subtitles_advanced_bp.route('/api/subtitles/priority/category/<category>', methods=['DELETE'])
def delete_category_priority(category):
    """Remove category-specific priority override"""
    config = _load_config()
    category_overrides = config.get('category_overrides', {})
    
    if category in category_overrides:
        del category_overrides[category]
        config['category_overrides'] = category_overrides
        _save_config(config)
        
        return jsonify({
            'ok': True,
            'message': f'Priority override for category "{category}" removed'
        })
    
    return jsonify({'ok': False, 'error': 'Category override not found'}), 404

@subtitles_advanced_bp.route('/api/subtitles/search', methods=['POST'])
